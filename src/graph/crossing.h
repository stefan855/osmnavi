...
class CrossingHelper {
 public:
  ...
  CrossingHelper(const Graph& g, TrafficSide traffic_side, const N3Path& n3p)
      : g_(g), traffic_side_(traffic_side), n3p_(n3p) {
    AddConnections();
  }
    ...
    for (uint16_t in_pos = 2; in_pos < conns_.size(); ++in_pos) {
      ...
      if (in_leg.bearing == in().bearing) {
        ...
        LOG_S(INFO) << "Additional IN conn with same angle node "
                    << n3p_.node1(g_).node_id;
        for (const GEdge& e : gnode_all_edges(g_, n3p_.node1_idx)) {
          LOG_S(INFO) << "  Edge node " << n3p_.node1(g_).node_id << ":"
                      << debug_str(g_, e);
        }
        for (const auto& conn : conns_) {
          LOG_S(INFO) << "  Connection node " << n3p_.node1(g_).node_id << ":"
                      << conn.DebugStr(g_);
        }
        continue;
      }
      for (uint16_t out_pos = 1; out_pos < conns_.size(); ++out_pos) {
        ...
        if (out_leg.dir != Outgoing || out_leg.node_idx == in_leg.node_idx) {
          // Not an outgoing edge or a u-turn.
          continue;
        }
        // Check for conflict.
        if (out_leg.bearing == out().bearing) {
          ...
          CHECK_NE_S(in_side, None) << n3p_.DebugStr(g_);
          CHECK_NE_S(out_side, None) << n3p_.DebugStr(g_);
          if (in_side != out_side) {
            conflicts->push_back({.in_conn_pos = in_pos,
                                  .out_conn_pos = out_pos,
                                  .type = Cross,
 ...
 private:
  ...
  bool HasHigherPriority(const ConflictPath& confl) const {
    ...
    if (in().hw == confl_in.hw ||
      ...
      return (traffic_side_ == TRAFFIC_SIDE_RIGHT) ==
             (confl.from_side == Right);
    } else {
  ...
  inline uint32_t GetBestHighwayEdge(uint32_t from_idx, uint32_t to_idx) {
    ...
    uint32_t idx = INFU32;
    for (const GEdge& e : gnode_forward_edges(g_, from_idx)) {
      if (e.target_idx == to_idx && g_.ways.at(e.way_idx).highway_label < hw) {
        hw = g_.ways.at(e.way_idx).highway_label;
        idx = gnode_edge_idx(g_, e);
      }
    }
    return idx;
  }
  ...
  uint16_t ComputeRank(Dir dir, uint16_t bearing) {
    ...
    // Add 1 if
    //   TRAFFIC_SIDE_RIGHT and direction is outgoing,
    // or
    //   TRAFFIC_SIDE_LEFT and direction is incoming.
    return out_bearing * 10 +
           (((traffic_side_ == TRAFFIC_SIDE_RIGHT) == (dir == Outgoing)) ? 1
                                                                         : 0);
  }
  ...
  void MayAddInConn(uint32_t node_idx) {
    ...
    const uint32_t e_idx = GetBestHighwayEdge(node_idx, n3p_.node1_idx);
    if (e_idx != INFU32) {
      const GEdge& e = g_.edges.at(e_idx);
      /*
      LOG_S(INFO) << absl::StrFormat(
          "ZZ1 IN connection %lu->%lu start_bearing:%u target_bearing:%u",
          GetGNodeIdSafe(g_, node_idx), GetGNodeIdSafe(g_, n3p_.node1_idx),
  ...
  void MayAddOutConn(uint32_t node_idx) {
    ...
    const uint32_t e_idx = GetBestHighwayEdge(n3p_.node1_idx, node_idx);
    if (e_idx != INFU32) {
      const GEdge& e = g_.edges.at(e_idx);
      /*
      LOG_S(INFO) << absl::StrFormat(
          "ZZ2 OUT connection %lu->%lu start_bearing:%u target_bearing:%u",
          GetGNodeIdSafe(g_, n3p_.node1_idx), GetGNodeIdSafe(g_, node_idx),
          e.start_bearing, e.target_bearing);
      */
      conns_.push_back({.node_idx = node_idx,
                        .dir = Outgoing,
  ...
  // 0/1.
  void AddConnections() {
    MayAddInConn(n3p_.node0_idx);   // Incoming edge is at position 0.
    MayAddOutConn(n3p_.node2_idx);  // Outgoing edge is at position 1.
    CHECK_EQ_S(conns_.size(), 2) << n3p_.DebugStr(g_);

    // Add all other connections.
    const uint32_t middle_idx = n3p_.node1_idx;
    for (const GEdge& e : gnode_all_edges(g_, middle_idx)) {
  ...
  const Graph& g_;
  const TrafficSide traffic_side_;
  const N3Path& n3p_;
};

inline DurationMS CrossingCost(const Graph& g, TrafficSide traffic_side,
                               VEHICLE vt, const N3Path& n3p, bool debug) {
  DurationMS cost(0u);
  ...
  const CrossingHelper hlp(g, traffic_side, n3p);
