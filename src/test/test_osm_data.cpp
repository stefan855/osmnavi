...
void TestSimpleTurnRestrictionParsing() {
...

  {
    ...
    CHECK_EQ_S(tr.path.front().from_node_idx, B);
    CHECK_EQ_S(tr.path.front().way_idx, Way0);
    CHECK_EQ_S(tr.path.front().to_node_idx, D);
    CHECK_EQ_S(tr.path.back().from_node_idx, D);
    CHECK_EQ_S(tr.path.back().to_node_idx, E);
    CHECK_EQ_S(tr.path.back().way_idx, Way2);
  }
  {
    OSMPBF::Relation rel = CreateSimpleTRRelation(g, &w, /*id=*/1, Way0, D,
                                                  Way2, "only_right_turn");
    TRResult res;
    ParseTurnRestriction(g, w.tagh, rel, Verbosity::Trace, &res);
    CHECK_EQ_S(res.num_success, 1);
    CHECK_EQ_S(res.num_error, 0);
  ...
  {
    OSMPBF::Relation rel =
        CreateSimpleTRRelation(g, &w, /*id=*/1, Way0, D, Way2, "no_exit");
    ...
    CHECK_EQ_S(tr1.path.size(), 2);
    CHECK_EQ_S(tr1.path.back().to_node_idx, E);
    CHECK_EQ_S(tr1.path.back().way_idx, Way2);
    CHECK_S(tr1.forbidden);
    CHECK_S(tr1.direction == TurnDirection::NoExit);
    const TurnRestriction& tr2 = res.trs.back();
    CHECK_EQ_S(tr2.path.size(), 2);
    CHECK_EQ_S(tr2.path.back().to_node_idx, F);
    CHECK_EQ_S(tr2.path.back().way_idx, Way3);
    CHECK_S(tr2.forbidden);
    CHECK_S(tr2.direction == TurnDirection::NoExit);
  }
  {
    OSMPBF::Relation rel =
        CreateSimpleTRRelation(g, &w, /*id=*/1, Way0, D, Way2, "no_entry");
    // no_exit can have multiple to-ways.
    ...
    const TurnRestriction& tr1 = res.trs.front();
    CHECK_EQ_S(tr1.path.size(), 2);
    CHECK_EQ_S(tr1.path.front().from_node_idx, B);
    CHECK_EQ_S(tr1.path.front().way_idx, Way0);
    CHECK_S(tr1.forbidden);
    CHECK_S(tr1.direction == TurnDirection::NoEntry);
    const TurnRestriction& tr2 = res.trs.back();
    CHECK_EQ_S(tr2.path.size(), 2);
    CHECK_EQ_S(tr2.path.front().from_node_idx, C);
    CHECK_EQ_S(tr2.path.front().way_idx, Way1);
    CHECK_S(tr2.forbidden);
    CHECK_S(tr2.direction == TurnDirection::NoEntry);
  }
  {
    OSMPBF::Relation rel =
        CreateSimpleTRRelation(g, &w, /*id=*/1, Way0, D, Way2, "no_blabla");
    TRResult res;
    ParseTurnRestriction(g, w.tagh, rel, Verbosity::Trace, &res);
...
void TestSimpleCombined() {
  ...
  }
  {
    ...
    SimpleTurnRestrictionMap m =
        ComputeSimpleTurnRestrictionMap(g, Verbosity::Trace, res.trs);
    CHECK_EQ_S(m.size(), 2);
    const TurnRestriction::TREdge key1 = {B, Way0, D};
    const TurnRestriction::TREdge key2 = {C, Way1, D};
    CHECK_S(m.contains(key1));
    CHECK_S(m.contains(key2));
    const auto& data1 = m[key1];
    const auto& data2 = m[key2];
    LOG_S(INFO) << "Simple TR1: "
                << SimpleTurnRestrictionDebugString(g, key1, data1);
    std::vector<uint64_t> expected1 = {'C', 'F'};
    CHECK_S(SimpleToNodeIds(g, key1, data1) == expected1);
    LOG_S(INFO) << "Simple TR2: "
                << SimpleTurnRestrictionDebugString(g, key2, data2);
