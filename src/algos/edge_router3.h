...
namespace {
const static TurnCostData g_turn_costs_all_good(32, TURN_COST_ZERO_COMPRESSED);
}
...
class EdgeRouter3 {
 public:
  ...
  EdgeRouter3(const Graph& g, int verbosity = 1)
      : g_(g),
        pq_(&MetricCmp),
  ...
  std::string AlgoName(const RoutingOptions& opt) {
    return opt.use_astar_heuristic ? "e3_astar" : "e3_dijkstra";
  }
  ...
  RoutingResult Route(std::uint32_t start_idx, std::uint32_t target_idx,
    ...
    CHECK_S(!opt.backward_search);  // not supported (anymore).
    Clear();
    ...
    while (!pq_.empty()) {
      ...
      const uint32_t target_node_idx = ve.key.GetToIdx(g_, ctr_list_);
      if (target_node_idx == target_idx) {
        // This edge ends at the target node.
        target_best_metric = ve.min_metric;
        target_best_v_idx = qe.v_idx;
        break;
      }

      // Add new edges to q, with min_metric = turn_cost + edge-metric
      ExpandForwardEdges(ctx, {.target_idx = target_node_idx,
      ...
      for (const VisitedEdge& ve : visited_edges_) {
        if (ve.shortest_route == (output_shortest > 0)) {
          ...
          myfile << absl::StrFormat("line,%s,%.6f,%.6f,%.6f,%.6f\n",
                                    ...
                                    a.ll.lat.AsDouble(), a.ll.lon.AsDouble(),
                                    b.ll.lat.AsDouble(), b.ll.lon.AsDouble());
        }
      }
    }
    myfile.close();
 ...
 private:
  ...
  struct Context {
    ...
    const LatE6 target_lat;
    const LonE6 target_lon;
  ...
  std::string VisitedEdgeToString(const VisitedEdge& ve) {
    std::string tr_str;
    if (ve.key.GetType() == EdgeRoutingLabel3::COMPLEX_TURN_RESTRICTION) {
      tr_str =
  ...
  std::uint32_t FindOrAddVisitedEdge(EdgeRoutingLabel3 key,
  ...
  std::optional<EdgeRoutingLabel3> CreateNextEdgeKey(
      const Context& ctx, const EdgeRoutingLabel3& prev_key, uint32_t start_idx,
      const GEdge& next_edge, bool next_in_target_area) {
    const bool active_key =
        prev_key.GetType() == EdgeRoutingLabel3::COMPLEX_TURN_RESTRICTION;
    ...
    if (!active_key && !next_edge.complex_turn_restriction_trigger) {
      ...
      return EdgeRoutingLabel3::CreateGraphEdgeLabel(g_, start_idx, next_edge,
                                                     next_in_target_area);
    }
  ...
  void InitialiseRoutingFromNode(const Context& ctx, uint32_t start_idx) {
    ...
    for (uint64_t off = 0; off < start_node.num_forward_edges; ++off) {
      ...
      EdgeRoutingLabel3 edge_label = EdgeRoutingLabel3::CreateGraphEdgeLabel(
  ...
  void ExpandForwardEdges(const Context& ctx, const PrevEdgeData prev) {
    const GNode& expansion_node = g_.nodes.at(prev.target_idx);
    const EdgeRoutingLabel3 prev_key = visited_edges_.at(prev.v_idx).key;
    const GEdge& prev_ge = prev_key.GetEdge(g_, ctr_list_);

#if 0
    // Precompute if we want to expand consider all edges leaving expansion node
    // or not. in non-hybrid mode we always want to consider all edges.
    const bool consider_all_edges =
        (!ctx.opt.hybrid.on || expansion_node.dead_end ||
         ...
         (expansion_node.cluster_id == ctx.opt.hybrid.start_cluster_id &&
          !prev_ge.cross_cluster_edge()));
#endif

    const TurnCostData& turn_costs = g_.turn_costs.at(prev_ge.turn_cost_idx);
    CHECK_EQ_S(turn_costs.turn_costs.size(), expansion_node.num_forward_edges);
    ...
    for (uint64_t offset = 0; offset < expansion_node.num_forward_edges;
      ...
      if (turn_costs.turn_costs.at(offset) == TURN_COST_INFINITY_COMPRESSED) {
        if (verbosity_ >= 3) {
          LOG_S(INFO) << absl::StrFormat(
              "REJ1  Edge from:%10lld to:%10lld infinite costs",
              expansion_node.node_id, GetGNodeIdSafe(g_, curr_ge.target_idx));
        }
      ...
      const WaySharedAttrs& wsa = GetWSA(g_, curr_ge.way_idx);
      if (RoutingRejectEdge(g_, ctx.opt, expansion_node, prev.target_idx,
                            curr_ge, wsa, EDGE_DIR(curr_ge))) {
        if (verbosity_ >= 3) {
          LOG_S(INFO) << absl::StrFormat(
              "REJ2  Edge from:%10lld to:%10lld routing reject",
              expansion_node.node_id, GetGNodeIdSafe(g_, curr_ge.target_idx));
        }
        continue;
      }
      ...
      if (!CheckRestrictedAccessTransition(prev, curr_ge,
        ...
        if (verbosity_ >= 3) {
          LOG_S(INFO) << absl::StrFormat(
              "REJ3  Edge from:%10lld to:%10lld restricted area trans",
              expansion_node.node_id, GetGNodeIdSafe(g_, curr_ge.target_idx));
        }
        continue;
      }
      ...
      // restrictions.
      const std::optional<EdgeRoutingLabel3> next_edge_key = CreateNextEdgeKey(
          ctx, prev_key, prev.target_idx, curr_ge, next_in_target_area);
      if (!next_edge_key.has_value()) {
        if (verbosity_ >= 3) {
          LOG_S(INFO) << absl::StrFormat(
              "REJ4  Edge from:%10lld to:%10lld create next edge key",
              expansion_node.node_id, GetGNodeIdSafe(g_, curr_ge.target_idx));
        }
        continue;
      }
    ...
    if (ctx.opt.hybrid.on && prev_ge.cross_cluster_edge &&
        expansion_node.cluster_id != ctx.opt.hybrid.target_cluster_id) {
      if (verbosity_ >= 3) {
        LOG_S(INFO) << "EXPA* node:" << expansion_node.node_id
                    << " cluster:" << expansion_node.cluster_id;
      ...
      // CHECK_S(!visited_edges_.at(prev.v_idx).key.IsClusterEdge());

      // Expect visited edge without complex turn restrictions attached, because
      // CTRs should not exists on cluster border edges.
      const VisitedEdge& prev_ve = visited_edges_.at(prev.v_idx);
      CHECK_S(prev_ve.key.GetType() !=
              EdgeRoutingLabel3::COMPLEX_TURN_RESTRICTION)
          << VisitedEdgeToString(prev_ve);

      // 'prev' is an edge that arrives at a border node of an in-between
      // cluster, i.e. a cluster that is not start or target. In this case we
      // can travel through the cluster.

      const GCluster& cluster = g_.clusters.at(expansion_node.cluster_id);
      const uint32_t in_edge_idx = prev_ve.key.GetEdgeIdx(g_, ctr_list_);
      const std::vector<std::uint32_t>& v_dist =
          cluster.GetEdgeOutDistances(in_edge_idx);

      for (uint32_t i = 0; i < cluster.border_out_edges.size(); ++i) {
        const GCluster::EdgeDescriptor& ed = cluster.border_out_edges.at(i);
        // Exit edge should not go to entry point (by construction), because it
        // would be an cluster-internal edge.
        CHECK_S(g_.edges.at(ed.g_edge_idx).target_idx != prev.target_idx);

        // BIG TODO: Direct u-turn at arriving border node may make sense, but
        // turn it off for now.
        if (ed.g_from_idx == prev.target_idx) {
          if (verbosity_ >= 3) {
            LOG_S(INFO) << "REJ*  " << "u-turn (TODO)";
            continue;
          }
        }
        ...
        EdgeRoutingLabel3 label = EdgeRoutingLabel3::CreateClusterEdgeLabel(
            g_, in_edge_idx, i, /*bit=*/false);
        std::uint32_t v_idx =
            FindOrAddVisitedEdge(label, ctx.opt.use_astar_heuristic);
