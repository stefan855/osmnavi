...
bool FindMMNode(const MMGraph& mmheader, int64_t id, MMFullNode* fn) {
  if (mmheader.find_node_by_id(id, fn)) {
    ...
    for (uint32_t edge_idx : mmc.edge_indices(fn->node_idx)) {
      const MMEdge e(mmc.edges.at(MEdgeIdx(edge_idx)));
...
void ComputeShortestMMPathsForCluster(MMGraph* mmgptr, uint32_t cluster_id) {
  ...
  LOG_S(INFO) << "Shortest mmpaths done cluster " << cluster_id;
}

void ComputeShortestMMPaths(int n_threads, MMGraph* mmgptr) {
  FUNC_TIMER()
  ThreadPool pool;
  for (uint32_t cluster_id = 0; cluster_id < mmgptr->clusters.size();
       ++cluster_id) {
    pool.AddWork([mmgptr, cluster_id](int thread_idx) {
      ComputeShortestMMPathsForCluster(mmgptr, cluster_id);
    });
  }
...
int main(int argc, char* argv[]) {
                     ...
                     check_mmgraph);
  {
    ...
    // Check.
    for (const MMCluster& mmc : mmg.clusters.span()) {
      const auto& expected_metrics =
          g.clusters.at(mmc.cluster_id).edge_distances;
      CHECK_EQ_S(expected_metrics.size(), mmc.in_edges.size());
      for (uint32_t idx1 = 0; idx1 < mmc.in_edges.size(); ++idx1) {
        for (uint32_t idx2 = 0; idx2 < mmc.out_edges.size(); ++idx2) {
          uint32_t ma = expected_metrics.at(idx1).at(idx2);
    ...
    if (mmg.clusters.size() > 20) {
      ...
      start_anchor.AddStartNode(mc10, MNodeIdx(mc10.nodes.size() / 2));

      const MMCluster& mc15 = mmg.clusters.at(15);
      GeoAnchor target_anchor;
      target_anchor.AddTargetNode(mc15, MNodeIdx(mc15.nodes.size() / 2));
