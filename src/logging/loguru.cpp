	...
	*/
	...
	static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
	{
		...
		if (custom_level_name) {
			...
			snprintf(level_buff, sizeof(level_buff) - 1, "%d", static_cast<int8_t>(verbosity));
		}

		size_t pos = 0;

		if (g_preamble_verbose && pos < out_buff_size && level_buff[0] != 0) {
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%c",
			                     level_buff[0]);
			if (bytes > 0) {
				pos += bytes;
			}
      ...
      */
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%s:%u ",
